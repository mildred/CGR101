...
set autoTriggerTimeouts {500 500 500 500 500 500 500 500 500 500 500 500 500 500 500 500 500 1000 2000 2000}
...
lappend timebaseImages [image create photo -file "$images/1us.gif"]
...
lappend timebaseImages [image create photo -file "$images/10us.gif"]
...
proc scope::adjustTimebase {direction} {
	...
	switch $direction {
		"out" {
			...
			if {$scope::timebaseIndex > 19} {
				set scope::timebaseIndex 19
			}
		} "in" {
...
proc scope::convertSample {sample channel} {
	...
	if {($channel =="a") || ($channel=="A")} {
		if {$scope::verticalIndexA <4} {
			set voltage [expr {(511-$sample)*$scope::stepSizeALow}]
		} elseif {$scope::verticalIndexA < 7} {
			if {$scope::probeA == 1.0} {
	...
	if {($channel == "b") || ($channel=="B")} {
		if {$scope::verticalIndexB <4} {
			set voltage [expr {(511-$sample)*$scope::stepSizeBLow}]
		} elseif {$scope::verticalIndexB < 7} {
			if {$scope::probeB == 1.0} {
