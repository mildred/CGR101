...
proc showChangeLog {} {
	...
	toplevel .changeLog
	wm title .changeLog "CGR-101 Change Log"
...
proc loadSettings {} {
	...
	set wave::waveFile [lindex $settings 10]
	...
	set wave::sliderMode [lindex $settings 11]
	
	#Restore digital outputs
	if {[lindex $settings 12]} {digio::toggleOutBit 0}
	if {[lindex $settings 13]} {digio::toggleOutBit 1}
	if {[lindex $settings 14]} {digio::toggleOutBit 2}
	if {[lindex $settings 15]} {digio::toggleOutBit 3}
	if {[lindex $settings 16]} {digio::toggleOutBit 4}
	if {[lindex $settings 17]} {digio::toggleOutBit 5}
	if {[lindex $settings 18]} {digio::toggleOutBit 6}
	if {[lindex $settings 19]} {digio::toggleOutBit 7}

	#Restore interrupt settings
	set digio::intMode [lindex $settings 20]
	digio::selectIntMode

	#Restore PWM settings
	set digio::pwmDuty [lindex $settings 21]
	digio::updatePWM $digio::pwmDuty
	set digio::pwmFreq [lindex $settings 22]
	digio::selectFreq
